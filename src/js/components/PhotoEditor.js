export default class PhotoEditor {
  ...
  slideZoom(clicks) {
    ...
    let pt = this.mouseOnCanvas
      ? this.ctx.transformedPoint(this.lastX, this.lastY)
      : this.ctx.transformedPoint(this.defaultX, this.defaultY);
    this.ctx.setTransform(1, 0, 0, 1, 0, 0);
  ...
  zoom(clicks) {
    ...
    if (decision === true) this.ctx.scale(factor, factor);
    this.handleZoom(scale * factor - 1);
    this.ctx.translate(-pt.x, -pt.y);
    this.redraw();
  }
  handleResize() {
