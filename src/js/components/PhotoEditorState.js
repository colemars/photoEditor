...
export default class PhotoEditor extends React.Component {
  ...
  redraw() {
    ...
    var p2 = this.state.ctx.transformedPoint(
    ...
    this.state.ctx.drawImage(
      this.state.gkhead,
      this.state.canvas.width / 2 - this.state.gkhead.width / 2,
  ...
  zoom(clicks) {
    var pt = this.state.ctx.transformedPoint(
      this.state.lastX,
      this.state.lastY
    );
  ...
  handleMouseDown(event) {
    document.body.style.mozUserSelect = document.body.style.webkitUserSelect = document.body.style.userSelect =
      "none";
    this.setState({
      lastX: event.offsetX || event.pageX - this.state.canvas.offsetLeft
    });
    this.setState({
      lastY: event.offsetY || event.pageY - this.state.canvas.offsetTop
    });
    this.setState({
      dragStart: this.state.ctx.transformedPoint(
        this.state.lastX,
        this.state.lastY
      )
    });
  ...
  handleMouseMove(event) {
    this.state.lastX =
      event.offsetX || event.pageX - this.state.canvas.offsetLeft;
    this.state.lastY =
      event.offsetY || event.pageY - this.state.canvas.offsetTop;
